...
def ResNet50(img_input):
    ...
    x = Conv2D(64, (3, 3), strides=(1, 1), padding='same', dilation_rate=(1, 1), name='conv1')(img_input)
    x = BatchNormalization(name='bn_conv1')(x)
    x = Activation('relu')(x)
    # 320,320,64 -> 320,320,64
    x = Conv2D(64, (3, 3), strides=(1, 1), padding='same', dilation_rate=(2, 2), name='conv2')(x)
    ...
    x = conv_block(x, 3, [64, 64, 128], stage=2, block='a', activation='relu', dilation_rate=(1, 1), strides=(2, 2))
    ...
    x = conv_block(x, 3, [128, 128, 256], stage=3, block='a', activation='relu', dilation_rate=(1, 1), strides=(2, 2))
    x = identity_block(x, 3, [128, 128, 256], stage=3, block='b', activation='relu', dilation_rate=(2, 2), se=False)
    x = identity_block(x, 3, [128, 128, 256], stage=3, block='c', activation='hardswish', dilation_rate=(4, 4), se=True)
    # x = identity_block(x, 3, [128, 128, 256], stage=3, block='d', activation='hardswish', dilation_rate=(2, 2), se=True)
    feat2 = x

    # 80,80,256 -> 40,40,512
    x = conv_block(x, 3, [256, 256, 512], stage=4, block='a', activation='hardswish', dilation_rate=(1, 1), strides=(2, 2))
    x = identity_block(x, 3, [256, 256, 512], stage=4, block='b', activation='hardswish', dilation_rate=(2, 2), se=True)
    x = identity_block(x, 3, [256, 256, 512], stage=4, block='c', activation='hardswish', dilation_rate=(4, 4), se=True)
